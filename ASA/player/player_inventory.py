...
def search_in_inventory(item:str):
    if is_open():
        ...
        x = variables.get_pixel_loc("search_inventory_x")
        ...
        for attempt in range(3):
            ...
            if template.verify_text_entered("search_inventory_active", timeout=1.0):
                ...
                logs.logger.warning(f"Player search text not verified (Attempt {attempt+1}/3). Retrying...")
                time.sleep(0.2)
                
        if not success:
            logs.logger.error(f"Skipping typing '{item}': Failed to verify text in player search bar after 3 attempts.")
...
def implant_eat():
    ...
    while not template.check_template("death_regions",0.7):
        ...
        # ==========================================
        # --- NEW: LOGIC-BASED FREEZE DETECTION ---
        # ==========================================
        if not is_open():
            logs.logger.critical(f"Suicide sequence failed to open inventory (Attempt {attempts}). UI is deadlocked.")
            if attempts >= ASA.config.suicide_attempts:
                logs.logger.critical("Maximum failsafes reached. The game is confirmed FROZEN. Force restarting ARK...")
                import reconnect.crash
                c = reconnect.crash.crash(windows.hwnd)
                c.re_open_game()
                return # Stop the loop, the game is now restarting!
        # ==========================================
        ...
        # Only try to click the implant if the inventory successfully opened
        if is_open():
